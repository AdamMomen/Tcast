...
import { publishCast, SignedCast, FarcasterContentHost, Farcaster } from '@standard-crypto/farcaster-js';
...
const publishCastMessage = async (message: string, privateKey: string): Promise<SignedCast | null> => {
    const provider = new AlchemyProvider("goerli", env.ALCHEMY_API_KEY || undefined);
    const wallet = Wallet.fromMnemonic(privateKey);

    return publishCast(wallet, provider, message)
}
...
const handler = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    try {
        ...
        if (platforms.farcaster) {
            const privateKey = getFCpkByTwitterId(twitterId)
            if (!privateKey) throw new ApiError(500, 'Couldn not find User\'s Farcaster Private Key')
            result = await publishCastMessage(text, privateKey)
        }
